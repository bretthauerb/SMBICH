// Power request handler qq driver
...
/* FinalState */			{InvalidAction,		InvalidAction,				InvalidAction},
	...
	while (TRUE)
		...
		case SendDeviceIrp:
...
#ifdef _DECIDE_FOR_WIN_98_OR_HIGHER_
...
#endif // #ifdef _DECIDE_FOR_WIN_98_OR_HIGHER_
			...
			// TM: what happens if status was not yet set ?????
			if (NT_SUCCESS(postatus))
				break;			// started device IRP okay

			KdPrint((DRIVER_NAME " - PoRequestPowerIrp failed - %X\n", postatus));
			action = CompleteMainIrp;
			ctx->status = postatus;
			continue;
			}					// SendDeviceIrp
...
#endif // #ifdef _DECIDE_FOR_NOT_FALSE
