								...
								IN PUNICODE_STRING RegistryPath)
{
	...
	{
		KdPrint(("SCH5627EMI - Expected version of WDM (%d.%2.2d) not available\n", 1, 0));
	...
	if (!servkey.Buffer)
	{
		KdPrint(("SCH5627EMI - Unable to allocate %d bytes for copy of service key name\n", RegistryPath->Length + sizeof(WCHAR)));
...
static NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT pdo)
	...
	WCHAR namebuf[32];
	_snwprintf(namebuf, arraysize(namebuf), L"\\Device\\SMBus0");
	...
	if (!NT_SUCCESS(status))
		...
		KdPrint(("SCH5627EMI - IoCreateDevice failed - %X\n", status));
		return status;
		}						// can't create device object
	...
	__try
		...
		RtlInitUnicodeString(&(pdx->uniSymbolicLinkName), L"\\DosDevices\\SMBus0" );

		status = IoCreateSymbolicLink(&(pdx->uniSymbolicLinkName), &devname);
		if ( !NT_SUCCESS(status) ) {
			KdPrint(("SCH5627EMI - Unable to create symbolic link\n"));
		...
		if (!pdx->devname.Buffer)
			...
			KdPrint(("SCH5627EMI - Unable to allocate %d bytes for copy of name\n", devname.MaximumLength));
			__leave;
			}					// can't allocate buffer
		...
		if (pdo)
		{
			...
			if (!pdx->LowerDeviceObject)
				{						// can't attach device
				KdPrint(("SCH5627EMI - IoAttachDeviceToDeviceStack failed\n"));
...
extern "C" void __declspec(naked) __cdecl _chkesp()
	{
	_asm je okay
	ASSERT("SCH5627EMI - Stack pointer mismatch!");
okay:
	_asm ret
	}
