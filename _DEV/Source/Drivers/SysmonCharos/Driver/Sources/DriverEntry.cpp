								...
								IN PUNICODE_STRING RegistryPath)
{
	...
	{
		KdPrint(("SysmonCharos.sys - Expected version of WDM (%d.%2.2d) not available\n", 1, 0));
	...
	if (!servkey.Buffer)
	{
		KdPrint(("SysmonCharos.sys - Unable to allocate %d bytes for copy of service key name\n", RegistryPath->Length + sizeof(WCHAR)));
...
static NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT pdo)
	...
	WCHAR namebuf[32];
	_snwprintf(namebuf, arraysize(namebuf), L"\\Device\\CHAROS");
	...
	if (!NT_SUCCESS(status))
		...
		KdPrint(("SysmonCharos.sys - IoCreateDevice failed - %X\n", status));
		return status;
		}						// can't create device object
	...
	__try
		...
		RtlInitUnicodeString(&(pdx->uniSymbolicLinkName), L"\\DosDevices\\CHAROS" );

		status = IoCreateSymbolicLink(&(pdx->uniSymbolicLinkName), &devname);
		if ( !NT_SUCCESS(status) ) {
			KdPrint(("SysmonCharos.sys - Unable to create symbolic link\n"));
		...
		if (!pdx->devname.Buffer)
			...
			KdPrint(("SysmonCharos.sys - Unable to allocate %d bytes for copy of name\n", devname.MaximumLength));
			__leave;
			}					// can't allocate buffer
		...
		if (pdo)
		{
			...
			if (!pdx->LowerDeviceObject)
				{						// can't attach device
				KdPrint(("SysmonCharos.sys - IoAttachDeviceToDeviceStack failed\n"));
...
extern "C" void __declspec(naked) __cdecl _chkesp()
	{
	_asm je okay
	ASSERT("!SysmonCharos.sys - Stack pointer mismatch!");
okay:
	_asm ret
	}
